...
function solution(strings, n) {
    ...
    for (let i = 0; i < strings.length; i++) { // 문자열 앞에 정렬로 기준을 할 문자 추가
        result[i] = strings[i].substring(n, n + 1) + strings[i];
    }

    result.sort(); // 정렬

    for (let i = 0; i < strings.length; i++) { // 문자열 앞에 정렬로 기준을 할 문자 제거
        result[i] = strings[i].slice(1, strings[i].length + 1);
    }
